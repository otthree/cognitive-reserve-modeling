...
class ADNI_3D_Custom(data.Dataset):

    def __init__(self, dir_to_scans, dir_to_tsv, mode = 'Train', n_label = 3, percentage_usage = 1.0, use_custom=True):
        if n_label == 3:
    ...
    def __getitem__(self, idx):
        try:
            # For custom data, use filepath directly from TSV
            if self.use_custom and 'filepath' in self.subject_tsv.columns:
                filepath = self.subject_tsv.iloc[idx].filepath
            ...
            cdr_sub = 0
            age = list(np.arange(0.0,120.0,0.5)).index(self.subject_tsv.iloc[idx].age_rounded)
